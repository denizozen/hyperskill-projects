    ...
    public void run() {
        ...
        while (!Thread.interrupted()) {
            if (System.currentTimeMillis() - searchTime > 1000 * 9) {
                checkResult = CheckResult.wrong("Couldn't find an authorization link with 'redirect_uri' within 9 seconds.");
                return;
            }

            String output = OutputHandler.getDynamicOutput();
            if (output.contains("redirect_uri=")) {
                redirectUri = output.split("redirect_uri=")[1];
        ...
        try {
            ...
            checkResult = CheckResult.wrong("Didn't receive any response from the server, found at 'redirect_uri': " + redirectUri);
        } catch (InterruptedException e) {
            checkResult = CheckResult.wrong("Request to " + redirectUri + " was interrupted. Make sure, that you give the right feedback in your browser.");
        } catch (Exception e) {
